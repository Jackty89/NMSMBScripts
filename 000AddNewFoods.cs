...
{
    private readonly IDictionary<string, string> RecipeCookingMethod = new Dictionary<string, string>
    {
        { "Stew",       "UI_COOK_STEW" },
        { "Drink",      "UI_COOK_DRINK" },
        { "Eggs",       "UI_COOK_EGGS" },
    ...
    private readonly IDictionary<string, string> NewFoodConsumeReward = new Dictionary<string, string>
    {
    ...
    private List<string> ProductsForCooking = new List<string>()
    {
        "CASING",
        "NIPNIPBUDS"
    };
    private List<string> SubstancesForCooking = new List<string>()
    {
        "LAUNCHSUB"
    };
    ...
    {
        List<CustomFood> customFoodList = new List<CustomFood>();
        customFoodList = CreateFoodList();

        foreach(CustomFood food in customFoodList)
            CreateFoods(food);
    ...
    protected List<CustomFood> CreateFoodList()
    {
        List<CustomFood> customFoods = new List<CustomFood>()
        {   
            new CustomFood
            {
                NewFoodID = "Tea",
                ...
                {
                    new(LanguageId.English, "Tea", "A lively and mouth-drying effect on the tongue. Not bitter, but a clean and refreshing quality. This tea is renowned across the local region, and is made fresh from whatever the recipe is.")
                },
